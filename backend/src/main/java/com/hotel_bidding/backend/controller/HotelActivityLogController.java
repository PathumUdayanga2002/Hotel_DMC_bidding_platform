...
/**
 * Activity Log Controller for Hotel
 * Only accessible by Hotel users (both super admin and staff can view their own company's logs)
 */
@RestController
@RequestMapping("/hotel/activity-logs")
@RequiredArgsConstructor
@Slf4j
@CrossOrigin(origins = "${app.frontend.url:http://localhost:5173}", allowCredentials = "true")
public class HotelActivityLogController {
    ...
    public ResponseEntity<ApiResponse> getActivityLogs(
        ...
        log.info("Hotel fetching activity logs - Page: {}, Size: {}", page, size);
    ...
    public ResponseEntity<ApiResponse> getActivityLogsByDateRange(
        ...
        log.info("Hotel fetching activity logs for date range: {} to {}", startDate, endDate);
        String userId = authentication.getName();
